...
class FlickrAPI {
    ...
    private class func flickrURL(method method: Method, parameters: [String:String]?) -> NSURL {
    ...
    class func photosFromJSONData(data: NSData, inContext context: NSManagedObjectContext) -> PhotosResult {
        do {
            let jsonObject: AnyObject = try NSJSONSerialization.JSONObjectWithData(data, options: [])
            guard let jsonDictionary = jsonObject as? [NSObject:AnyObject],
                photos = jsonDictionary["photos"] as? [String:AnyObject],
                photosArray = photos["photo"] as? [[String:AnyObject]] else {
                    //the JSON structure doesnt match expectations
            ...
            if finalPhotos.count == 0 && photosArray.count > 0 {
                //we weren't able to parse any photos, maybe JSON has changed?
                return .Failure(FlickrError.InvalidJSONData)
    ...
    private class func photosFromJSONObject(json: [String: AnyObject], inContext context: NSManagedObjectContext) -> Photo? {
        guard let
